...
export default function DemoChatInterface() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#0f172a] to-[#0ea5e9] flex items-center justify-center p-4 text-[#f8fafc]">
      <div className="w-full max-w-md">
        <div className="bg-[#101820] rounded-3xl shadow-2xl overflow-hidden backdrop-blur-sm bg-opacity-95">
          {/* Header */}
          <div className="bg-gradient-to-r from-[#0ea5e9] to-[#059669] text-white p-6 text-center">
            <h1 className="text-xl font-bold text-[#f8fafc]">{clinic.practice_name}</h1>
            <p className="text-[#dbeafe] text-sm mt-1">Dr. {clinic.doctor_name} • {clinic.specialty}</p>
            {/* Activity indicator */}
            <div className="mt-3 bg-white/20 rounded-full px-3 py-1 text-xs">
              <span className="inline-block w-2 h-2 bg-[#059669] rounded-full mr-2 animate-pulse"></span>
              {t.activityStatus}
            </div>
          </div>
          {/* Chat Messages */}
          <div className="h-96 overflow-y-auto p-4 space-y-4">
              ...
              <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl ${
                  msg.role === 'user' 
                    ...
                    : 'bg-[#f8fafc] text-[#101820]'
                }`}>
                  <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
                  <p className={`text-xs mt-1 ${
                    msg.role === 'user' ? 'text-[#dbeafe]' : 'text-[#6b7280]'
                  }`}>
            ...
            {isAiTyping && (
              <div className="flex justify-start">
                <div className="bg-[#f8fafc] text-[#101820] px-4 py-2 rounded-2xl">
            ...
            <div className="flex flex-wrap gap-2">
              {getSuggestedPrompts().slice(0, 2).map((prompt, index) => (
                <button
                  ...
                  className="text-xs bg-gradient-to-r from-[#e0f7fa] to-[#bbf7d0] text-[#0ea5e9] px-3 py-1 rounded-full hover:from-[#b6e0fe] hover:to-[#a7f3d0] transition-colors"
                >
                  💡 {prompt}
                </button>
              ))}
          ...
          <div className="p-4 border-t border-[#1e293b]">
            <div className="flex space-x-2">
              <input
                ...
                className="flex-1 border border-[#0ea5e9] rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0ea5e9] focus:border-transparent bg-[#101820] text-[#f8fafc] placeholder-[#6b7280]"
                disabled={isAiTyping}
              />
              <button
                onClick={sendMessage}
                disabled={!message.trim() || isAiTyping}
                className="bg-gradient-to-r from-[#0ea5e9] to-[#059669] text-white px-6 py-2 rounded-full hover:from-[#2563eb] hover:to-[#059669] disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm font-medium"
              >
                {t.send}
              </button>
            </div>
            ...
            <div className="flex justify-center mt-2">
              <button
                ...
                className="text-xs text-[#6b7280] hover:text-[#0ea5e9] underline"
              >
                {t.clearMessages}
              </button>
            </div>
