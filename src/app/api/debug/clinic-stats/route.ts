...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🏥 Fetching clinic info for ID:', clinicIdNum);
    ...
    if (clinicError) {
      ...
      return createErrorResponse('Clinic database error', 401, clinicError?.message);
    }
    
    if (!clinic) {
      console.log('❌ No clinic found with ID:', clinicIdNum);
      return createErrorResponse('Clinic not found', 404);
    }
    
    console.log('✅ Clinic found:', clinic.practice_name);
