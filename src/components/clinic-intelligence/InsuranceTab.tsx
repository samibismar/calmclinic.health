...
import { CreditCard, DollarSign, Plus, Edit, Trash2, Save, X } from "lucide-react";
...
const planTypes = [
  { id: 'major', name: 'Major Insurance', color: 'bg-blue-500' },
  { id: 'medicare', name: 'Medicare', color: 'bg-green-500' },
  { id: 'medicaid', name: 'Medicaid', color: 'bg-orange-500' },
  { id: 'private', name: 'Private Pay', color: 'bg-purple-500' }
];
...
export default function InsuranceTab() {
  ...
  const getPlansByType = (type: string) => {
    return insurancePlans.filter(plan => plan.plan_type === type && plan.is_active);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold text-white">Insurance & Billing</h2>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {planTypes.map((type) => {
            const typePlans = getPlansByType(type.id);
            
            return (
              <div key={type.id} className="bg-white/5 border border-white/20 rounded-lg p-6">
      ...
      <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-6">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <h3 className="text-lg font-semibold text-white">Billing Policies</h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white/5 border border-white/20 rounded-lg p-4">
            <h4 className="text-sm font-medium text-white mb-2">Payment Requirements</h4>
            <ul className="text-sm text-blue-200 space-y-1">
              <li>• Deductibles due at time of service</li>
              <li>• Copays due at time of service</li>
              <li>• Refraction fee: $25 (not covered by insurance)</li>
            </ul>
          </div>
          ...
          <div className="bg-white/5 border border-white/20 rounded-lg p-4">
            ...
            <ul className="text-sm text-blue-200 space-y-1">
              <li>• Missed appointment fee: $25</li>
          ...
          <div className="bg-gradient-to-br from-blue-950 to-blue-900 rounded-xl p-6 w-full max-w-md border border-white/20">
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  {planTypes.map((type) => (
                    <option key={type.id} value={type.id} className="bg-blue-900 text-white">
                      {type.name}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent"
                  placeholder="Any specific coverage details or restrictions..."
                />
              </div>
            </div>
